...
<div class="container">
    ...
    <div class="btn-group">
        <?php if (in_array('inclusao', $usuario_permissoes)): ?>
            <button class="btn" onclick="window.location.href='../admin/add/add_parto.php'">Adicionar novo Parto</button>
        <?php endif; ?>
    </div>
        ...
        <tr>
            ...
            <td>
                <?php if (in_array('alteracao', $usuario_permissoes)): ?>
                    <button class="btn" onclick="window.location.href='../admin/edit/edit_parto.php?id=<?php echo $row['id']; ?>'">Editar</button>
                <?php endif; ?>
                <?php if (in_array('exclusao', $usuario_permissoes)): ?>
                    <!-- O arquivo abaixo não foi criado - Letícia -->
                    <button class="btn" onclick="if(confirm('Deseja realmente excluir este parto?')) window.location.href='../admin/delete/delete_parto.php?id=<?php echo $row['id']; ?>'">Excluir</button>
                <?php endif; ?>
            </td>
        </tr>
        <?php endwhile; ?>
